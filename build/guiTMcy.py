...
label_Head = tk.Label(window, text="Circular Waveguide(TM)", bg="#D9D9D9",font=(10), relief="sunken")
...
c = 100  # Depth
m,g= 1,0 # TM_mn mode (m is an integer for cylindrical symmetry)
...
r = np.linspace(0.0001,default_R, Nr)
...
def update_plot(selected_plot):
    ...
    if selected_plot == 'Radial':
        ...
        ax1.contourf(Theta[:, :, Z_new, T_new], R[:, :, Z_new, T_new], hr[:, :, Z_new, T_new].real, cmap=cm.viridis)
        ...
        ax0.contourf(Theta[:, :, Z_new, T_new], R[:, :, Z_new, T_new], e0[:, :, Z_new, T_new].real, cmap=cm.viridis)
        ax0.set_title('Angular Electric Field Component')
        ax0.grid(0)
        ax0.set_rticks([default_R]) 
        ax0.set_rmax(default_R)
        ax1.contourf(Theta[:, :, Z_new, T_new], R[:, :, Z_new, T_new], h0[:, :, Z_new, T_new].real, cmap=cm.viridis)
        ax1.set_title('Angular Magnetic Field Component')
        ax1.grid(0)
        ax1.set_rticks([default_R]) 
        ax1.set_rmax(default_R)
        ...
        # print(m,g,k_c)
        ax0.contourf(Theta[:, :, Z_new, T_new], R[:, :, Z_new, T_new], ez[:, :, Z_new, T_new].real, cmap=cm.viridis)
        ax0.set_title('Electric Field Component along Z axis')
        ax0.grid(0)
        ax0.set_rticks([default_R]) 
        ax0.set_rmax(default_R)
        
        ax1.contourf(Theta[:, :, Z_new, T_new], R[:, :, Z_new, T_new], hz[:, :, Z_new, T_new].real, cmap=cm.viridis)
        ax1.set_title('Magnetic Field Component along Z axis')
        ax1.grid(0)
        ax1.set_rticks([default_R]) 
        ax1.set_rmax(default_R)
