...
label_Head = tk.Label(window, text="Rectangular Waveguide(TE)", bg="#D9D9D9",font=(10), relief="sunken")
...
def update_equations():
    global e_x, e_y, m_x, m_y, hz_values
    e_x = ex(U, V, W)
    e_y = ey(U, V, W)
    m_x = hx(U, V, W)
    m_y = hy(U, V, W)
    hz_values = Hz(X, Y, Z, T)
...
def ex(x, y,z):
    # return (np.exp(1j*np.pi/2) * omega * mu * np.pi * n / (400 * np.pi**2 * b)) *  np.cos(m * np.pi * x / a) * np.sin(n * np.pi * y/ b) * np.exp(-1j* beta * z)
    return (1j * omega * mu * np.pi * n / (8000)) *  np.cos(m * np.pi * x / a) * np.sin(n * np.pi * y/ b) * np.exp(-1j* beta * z)
def ey(x, y,z):
    # return -(np.exp(1j*np.pi/2) * omega * mu * np.pi * m / (400 * np.pi**2 * a)) *  np.sin(m * np.pi * x / a) * np.cos(n * np.pi * y / b) *  np.exp(-1j* beta * z)
    return -(1j * omega * mu * np.pi * m / (20000)) *  np.sin(m * np.pi * x / a) * np.cos(n * np.pi * y / b) *  np.exp(-1j* beta * z)
def hx(x, y,z):
    # return -(np.exp(1j*np.pi/2) * eta * lambda_ * (m * np.pi / a)) *  np.sin(m * np.pi * x / a) * np.cos(n * np.pi * y / b) * np.exp(-1j* beta * z)
    return -(1j * eta * lambda_ * (m * np.pi / a)) *  np.sin(m * np.pi * x / a) * np.cos(n * np.pi * y / b) * np.exp(-1j* beta * z)
def hy(x, y,z):
    # return np.exp(1j*np.pi/2) * eta * lambda_ * (n * np.pi / a) *  np.cos(m * np.pi * x / a) * np.sin(n * np.pi * y / b) * np.exp(-1j* beta * z)
    return 1j * eta * lambda_ * (n * np.pi / a) *  np.cos(m * np.pi * x / a) * np.sin(n * np.pi * y / b) * np.exp(-1j* beta * z)
...
plot_options = ['Hz', 'Ez', 'Hz_3d', 'contour_Hz']
selected_plot = tk.StringVar()
selected_plot.set('contour_Hz')
...
def update_plot(selected_plot):
    ...
    if selected_plot == 'Hz':
        plt.quiver(U[:, :,0], V[:, :,0], np.imag(m_x[:, :,0]), np.imag(m_y[:, :,0]), scale=1000, color='red')
        ...
        plt.quiver(U[:, :,0], V[:, :,0], np.imag(e_x[:, :,0]), np.imag(e_y[:, :,0]), scale=2000, color='blue')
        plt.title('Electric field(Ez)')
        plt.xlabel('breadth(a)')
        plt.ylabel('length(b)')
        plt.close()

    elif selected_plot == 'Hz_3d':
        ax = fig.add_subplot(111, projection='3d')
        ax.plot_surface(X[:, :, Z_new, T_new], Y[:, :, Z_new, T_new], np.imag(hz_values[:, :, Z_new, T_new]), cmap=cm.plasma, rstride=1, cstride=1, linewidth=0)
        ax.set_title('Magnetic field(3D) along z axis')
        plt.xlabel('breadth(a)')
        plt.ylabel('length(b)')
        ax.set_zlabel('Z Axia\n(depth)')
        plt.close()

    elif selected_plot == 'contour_Hz':
        plt.contourf(X[:, :, Z_new, T_new], Y[:, :, Z_new, T_new], np.real(hz_values[:, :, Z_new, T_new]), cmap=cm.plasma)
        plt.title('Magnitude plot of Magnetic field from end view')
        plt.xlabel('breadth(a)')
        plt.ylabel('length(b)')
        plt.close()
...
button_to_TM.place(x=19, y=58, width=100,height=50)
